...
def __preprocess_data(df):
    ...
    """
    df = df.drop(['raw_domain', 'sld', 'longest_word'], axis = 1)
    return df


def predict_rf(data, logger):
    """Loading the randomforest classifier predict_proba
    parameters:
        df: dataframe containing data to fit on
    ...
    model = pickle.load(open('../../models/rf_model.pkl', 'rb')) # load the pickling the model
